...
Also, for a real number $x$, the floor and ceiling functions are
denoted $\floor x=\max\left\{ i\in\ZZ:i\le x\right\} $ and $\ceil x=\min\left\{ i\in\ZZ:i\ge x\right\} $.
For a positive integer $n$, we write $\range n$ for the set $\left\{ 1,2,\dots,n\right\} $.
Finally, all logarithms are base $e$.
...
\]
\end{proof}
\ref{lem:num-extensions} then immediately follows from \ref{thm:number-of-completions-upper}
