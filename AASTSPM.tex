...
is, a $3$-uniform \emph{hypergraph} on the vertex set $\range n$),
...
random Steiner triple system. We have a general theorem summarizing
our method, which we state in \ref{sec:random-STS}.

A \emph{matching }in a hypergraph is a collection of disjoint edges,
and a \emph{perfect matching }is a matching covering the vertex set.
The existence of perfect matchings is one of the most central questions
in the theory of graphs and hypergraphs; in particular, one of the
most celebrated recent developments in the field is the Fulkerson-prize-winning
work of Johansson, Kahn and Vu \cite{JKV08} on perfect matchings
in random hypergraphs. A perfect matching in a Steiner triple system
...
important. Finally, all logarithms are base $e$.
...
\]
\end{lem}
\ref{lem:num-extensions} can be proved with only very slight adaptations
to the proof of \ref{eq:num-STS} in \cite{Kee15}. The details are
in \ref{sec:completion-count}. The point of \ref{lem:num-extensions}
...
See for example \cite{CT12} for an introduction to entropy and proofs
...
and therefore
\[
H\left(\randM_{v}\cond\randM_{v'}:\lambda_{v'}>\lambda_{v}\right)\le\E\left[\log\randR_{v}\left(\lambda\right)\right].
\]
