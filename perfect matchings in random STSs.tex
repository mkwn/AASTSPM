...
and therefore $H\left(\randM_{v}\cond\randM_{v'}:\lambda_{v'}>\lambda_{v}\right)\le\E\left[\log\randR_{v}\left(\lambda\right)\right]$.

Let $\randl=\left(\randl_{v}\right)_{v\in\range n}$ be a sequence
of independent random variables, where each $\randl_{v}$ has the
uniform distribution in $\left[0,1\right]$. (Note that almost surely
each $\randl_{v}$ is distinct). By linearity of expectation and the
tower law, 
\[
H\left(\randM\right)=\sum_{v\in\range n}\E\left[\log\randR_{v}\left(\randl\right)\right].
\]
...
so we will ignore this detail). Note that there are $\left(n-1\right)/2$
...
for more detailed exposition and a brief introduction to the notion
of entropy.
\begin{thm}
