...
Basically, we prove the a.a.s. existence of certain small substructures
that allow us to complete an almost-perfect matching into a perfect
one.
...
vertices and a flexible set $Z$ of size $3\floor{\delta^{2}n}$.
\item For at most $\delta n$ of the vertices $v\in V\backslash V\left(H\right)$,
we have $\left|\left\{ \left\{ x,y\right\} \subseteq Z:\left\{ v,x,y\right\} \in S\right\} \right|<2\delta^{3}n$.
...
\end{lem}
\begin{proof}
It is convenient to count \emph{ordered} perfect matchings, and then
divide by $\left(n/3\right)!$. We describe how to build an ordered
perfect matching edge-by-edge.

In a Steiner triple system every vertex is in $\left(n-1\right)/2$
hyperedges, and for small $\delta$ most of those hyperedges do not
involve a vertex of $H$. We can therefore greedily choose a matching
of size $\ceil{\delta n}$ in $V\backslash V\left(H\right)$ containing
each of the vertices violating the second condition. There are $n'=n-v\left(H\right)-\ceil{\delta n}=\left(1-O\left(\delta\right)\right)n$
...
in $V\backslash V\left(H\right)$ until there are only $\floor{\delta^{3}n}$
such vertices remaining unmatched. (This means we take a matching
with $m=\left(n'-\floor{\delta^{3}n}\right)/3=n/3-O\left(\delta n\right)$
...
\end{align*}
Note that this expression divided by $\left(n/3\right)!=\left(\left(1+o\left(1\right)\right)n/3e\right)^{n/3}$
gives \ref{eq:count-pm}, so it suffices to show that any matching
in $V\backslash V\left(H\right)$ covering all but $\floor{\delta^{3}n}$
vertices, none of which violate the second condition, can be completed
to a perfect matching.

By the second condition, for each uncovered $v\in V\backslash V\left(H\right)$
we can greedily extend our matching with a hyperedge containing $v$
and two vertices of $Z$, disjoint from the hyperedges chosen so far.
We can then repeatedly apply the third condition to the uncovered
vertices in $Z$ to extend our matching to cover half of $Z$. By
the defining property of a resilient template, there is a perfect
matching in the subgraph of the template induced by the vertices unmatched
so far. For each hyperedge of this perfect matching, use the covering
matching of the corresponding absorber; for each other hyperedge in
the template use the non-covering matching. These edges from the absorbers
allow us to complete our perfect matching of $S$.
\end{proof}
