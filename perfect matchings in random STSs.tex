...
\author{Matthew Kwan \thanks{Department of Mathematics, ETH, 8092 Zürich. Email: \href{mailto:matthew.kwan@math.ethz.ch} {\nolinkurl{matthew.kwan@math.ethz.ch}}.}}
...
\end{comment}
\begin{abstract}
We show that almost all Steiner triple systems have perfect matchings.
...
general theorem comparing a uniformly random Steiner system to the
outcome of a random clique removal process, which we hope will be
useful for other problems. We believe our methods can be easily adapted
to other types of designs, for example to show that almost all Latin
squares have transversals.
\end{abstract}

\section{Introduction}

A \emph{Steiner system} of order $n$ with parameters $\left(q,r\right)$,
$q>r$, is a collection $S$ of size-$q$ subsets of $\range n=\left\{ 1,\dots,n\right\} $
(that is, a $q$-uniform hypergraph on the vertex set $\range n$),
such that every size-$r$ subset of $\range n$ is included in exactly
one element (hyperedge) of $S$. Steiner systems are among the most
fundamental types of combinatorial designs, and have strong connections
to a wide range of different subjects, ranging from group theory,
to finite geometry, to experimental design, to the theory of error-correcting
codes. See \cite[Chapter~19]{VW01} for an introduction to the subject.

A $\left(q,1\right)$-Steiner system is a partition of $\range n$
into subsets of size $q$. In the language of hypergraphs, it is precisely
a $q$-uniform perfect matching. Steiner systems with all other combinations
of parameters are notoriously difficult to study. The simplest case
is that of $\left(3,2\right)$-Steiner systems, commonly known as
\emph{Steiner triple systems}. A Steiner triple system is actually
nothing more than a triangle-decomposition of the edges of the complete
graph $K_{n}$, so Steiner triple systems are natural ``symmetric''
counterparts to \emph{Latin squares}, which can be defined as triangle-decompositions
of the complete tripartite graph $K_{n,n,n}$.
...
property known to hold a.a.s. for random Steiner systems with any
combination of parameters. Following Erd\H os and R\'enyi's seminal
paper \cite{ER59} on random graphs and Erd\H os' popularization
of the probabilistic method, there have been great developments in
the theory of random combinatorial structures of all kinds, but essentially
none of the tools developed seem to be applicable to Steiner systems.
Steiner systems lack independence or any kind of recursive structure,
which rules out many of the techniques used to study Erd\H os-R\'enyi
random graphs and random permutations, and there is basically no freedom
to make local changes, which precludes the use of ``switching''
techniques often used in the study of random regular graphs (see for
example \cite{KSVW01}). It is not even clear how to study random
Steiner systems empirically; in an attempt to find an efficient algorithm
to generate a random Steiner triple system, Cameron \cite{Cam02}
designed a Markov chain on Steiner triple systems, but he was not
able to determine whether this chain was connected.

In a huge breakthrough that will surely revolutionize design theory,
Keevash \cite{Kee14} very recently proved that ``partial'' Steiner
systems satisfying a certain ``quasirandomness'' condition can be
completed into Steiner systems. He used this to settle a longstanding
existence question for Steiner systems, and shortly afterwards \cite{Kee15}
he proved new estimates for the number of Steiner systems with each
combination of parameters. In particular he proved that there are
\[
\left(n/e^{2}+o\left(n\right)\right)^{n^{2}/6}
\]
Steiner triple systems of order $n$, as long as $n$ satisfies necessary
divisibility conditions (Steiner triple systems can only exist if
$n$ is 1 or 3 mod 6; we will implicitly assume this for the rest
of the paper).
...
asymptotically smaller estimates on the number of Steiner triple systems
not satisfying a certain property. However, for most properties it
is not at all clear how to prove such estimates. Instead, we introduce
a way to use Keevash's methods to show that a uniformly random Steiner
triple system can in some sense be approximated by the outcome of
a random process called the \emph{triangle removal process}. We remark
that actually Keevash's lower bound is proved via a randomized construction
that involves the triangle removal process, so many properties that
hold a.a.s. in the triangle removal process trivially hold a.a.s.
in this random construction. Such results have been proved in \cite[Proposition~3.1]{LL15}
and \cite{LLR15}. However, the Steiner triple systems obtainable
by Keevash's construction comprise a negligible proportion of the
set of Steiner triple systems, so a more delicate approach is required
to study a uniformly random Steiner triple system. We give the details
in \ref{sec:outline}.

As an application of our new method, we prove that almost all Steiner
triple systems have many perfect matchings. The existence of perfect
matchings is one of the most central questions in the theory of graphs
and hypergraphs. In particular, one of the most celebrated recent
developments in the field is the Fulkerson-prize-winning work of Johansson,
Kahn and Vu \cite{JKV08} on the number of hyperedges a random hypergraph
should have for there to a.a.s. be a perfect matching. We remark that
Alon, Kim and Spencer \cite{AKS97} proved that actually every Steiner
triple system has an almost-perfect matching covering all but $o\left(\sqrt{n}\log^{3/2}n\right)$
vertices.
\begin{thm}
\label{thm:PM-in-STS}Let $\randS$ be a uniformly random order-$n$
Steiner triple system. Then a.a.s. $\randS$ contains
\[
...
by Krivelevich \cite{Kri97}). Basically, we prove the existence of
certain small substructures that allow us to complete an almost-perfect
...
columns and symbols, and so a triangle $\left(i,j,k\right)$ corresponds
...
have not yet been adapted to Latin squares, and it would be well outside
the scope of this paper to include such an adaptation here. Actually,
the author is currently working with Nina Kam\v cev, D\'aniel Kor\'andi,
Zur Luria, Frank Mousset and Alexey Pokrovskiy, on adapting Keevash's
methods to (a class of designs generalizing) Latin squares, which
would settle this issue. In any case, modulo a Keevash-type completion
result we can prove the following theorem, answering a question of
Glebov and Luria \cite{GL16} and proving that the Ryser-Brualdi-Stein
...
\begin{equation}
\left(\left(1-o\left(1\right)\right)\frac{n}{e^{2}}\right)^{n}\label{eq:num-STS}
\end{equation}
transversals.
\end{sol}
...
fact, we do everything in the general setting of $\left(q,r\right)$-Steiner
systems, because this does not introduce any difficulties). In \ref{sec:absorbers}
we define absorbers and prove \ref{thm:PM-in-STS}, in \ref{sec:max-num-matchings}
we prove \ref{thm:maximum-PMs}, and in \ref{sec:latin} we sketch
how \ref{thm:latin} may be proved. We have some concluding remarks,
including a long list of open problems, in \ref{sec:concluding}.
Finally, we have two appendices; in \ref{sec:keevash} we provide
a straightforward but necessary adaptation of Keevash's methods, to
estimate the number of completions of a partial Steiner system, and
in \ref{sec:Random-triangle-removal} we have a very simple analysis
of a clique removal process generalizing the triangle removal process.

\subsection{Acknowledgements}

The author would like to thank Asaf Ferber and Benny Sudakov for fruitful
discussions about random designs and the intricacies of the absorbing
method. Asaf and Benny could both have deservedly been coauthors on
this paper, but graciously declined the opportunity.

\section{\label{sec:outline}Random Steiner systems via clique removal processes}

In this section we describe our method for comparing random Steiner
triple systems with the outcome of the triangle removal process. Actually,
the situation for general $\left(q,r\right)$-Steiner systems is essentially
the same, so we state everything in general terms in case this is
useful in the future. For this entire section, $q$ and $r$ should
be considered fixed, and whenever we say ``Steiner system'' we mean
``$\left(q,r\right)$-Steiner system''. Let $Q={q \choose r}$ and
let $N={n \choose r}/Q=\left(1+o\left(1\right)\right)\left(\left(q-r\right)!/q!\right)n^{r}$
be the number of hyperedges in a Steiner system.

Before we can state our general theorem for comparing random Steiner
systems with clique removal processes, we need several definitions.
\begin{defn}[partial Steiner systems]
A \emph{partial Steiner system} (or \emph{partial system} for short)
is a $q$-uniform hypergraph on $\range n$ in which every $r$-set
of vertices is included in no more than one hyperedge. Let $\Sm m$
be the set of partial systems with $m$ hyperedges. We will also want
to consider partial systems with ordered hyperedges. Let $\ord$ be
the set of ordered Steiner systems, and let $\ordm m$ be the set
of ordered partial systems with $m$ hyperedges. For $S\in\ordm m$
and $i\le m$, let $S_{i}$ be the partial system consisting of the
first $i$ hyperedges of $S$. For a (possibly ordered) partial system
$S$, let $G\left(S\right)$ be the $r$-uniform hypergraph with an
edge for every pair of vertices which does not\emph{ }appear in any
hyperedge of $S$. So, if $S$ has $m$ hyperedges, then $G\left(S\right)$
has ${n \choose r}-Qm$ edges.
\end{defn}
%
\begin{defn}[quasirandomness]
For an $r$-graph $G$ with $m$ edges, let $d\left(G\right)=m/{n \choose r}$
denote its density. For an $\left(r-1\right)$-set of vertices $W$
in an $r$-graph $G$, the \emph{neighbourhood} $N_{G}\left(W\right)$
is the set of vertices $v$ such that $\left\{ v\right\} \cup W$
is an edge of $G$. We say $G$ is \emph{$\left(\varepsilon,h\right)$-quasirandom}
if for every set $A$ of $\left(r-1\right)$-sets of vertices with
$\left|A\right|\le h$, we have $\left|\bigcap_{W\in A}N_{G}\left(W\right)\right|=\left(1\pm\varepsilon\right)d\left(G\right)^{\left|A\right|}n$.
...
\end{defn}
%
\begin{defn}[the clique removal processs]
The clique removal process is defined as follows. Start with the
complete $r$-graph $K_{n}^{\left(r\right)}$ and iteratively delete
a copy of $K_{q}^{\left(r\right)}$ chosen uniformly at random from
all copies of $K_{q}^{\left(r\right)}$ in the remaining $r$-graph.
If we continue this process for $m$ steps, the deleted copies of
$K_{q}^{\left(r\right)}$ (in order) can be interpreted as an ordered
partial system in $\ordm m$. It is also possible that the process
aborts (because there are no copies of $K_{q}^{\left(r\right)}$ left)
...
If
\[
\Pr\left(\randS'\notin\mathcal{P}\cond\randS'\in\mathcal{Q}\right)<\exp\left(-n^{r-b}\right)
\]
then
\[
...
clique removal process is likely to produce quasirandom graphs; that
is, $\Pr\left(\randS'\in\mathcal{Q}\right)=1-o\left(1\right)$. However,
as we will see in \ref{subsec:absorbers}, the conditioning in \ref{lem:triangle-removal-transfer}
can still be useful because the probabilities under consideration
are so small (it is certainly not true that $\Pr\left(\randS'\notin\mathcal{Q}\right)$
is anywhere near as small as $\exp\left(-\Omega\left(n^{2}\right)\right)$).
...
\[
\frac{\left|\ext S\right|}{\left|\ext{S'}\right|}=\exp\left(n^{r-b}\right).
\]
\end{lem}
...
of any given Steiner system. We prove the following lemma in \ref{sec:random-is-typical}.
\begin{lem}
...
consider any Steiner system $S$, and uniformly at random order its
hyperedges to obtain an ordered Steiner system $\randS\in\ord$. Then
...
is roughly equally likely to be produced by the clique removal process,
...
and for $S,S'\in\cordm{n^{-a},q-1}{\alpha N}$ and $\randS\in\rg n{\alpha N}$,
\[
\frac{\Pr\left(\randS=S\right)}{\Pr\left(\randS=S'\right)}\le\exp\left(O\left(n^{r-a}\right)\right).
\]
\end{lem}
...
\subsection{Randomly ordered Steiner systems\label{sec:random-is-typical}}
...
Now, fix a set $A$ of $\left(r-1\right)$-sets of vertices with $\left|A\right|\le h$.
...
hyperedges of $S$ that include more than one set in $A$ (this follows
from the Steiner property, because the intersection of two sets in
$A$ has size at least $r$). Let $U$ be the set of vertices involved
in these atypical hyperedges, plus the vertices that appear in sets
in $A$, so that $\left|U\right|=O\left(1\right)$. Let $\randN=\left|\left(\bigcap_{W\in A}N_{\randG'}\left(W\right)\right)\backslash U\right|$.
...
$\randN$ by at most $q-1=O\left(1\right)$. So, by the Azuma-Hoeffding
...
\subsection{Approximate uniformity of the clique removal process\label{sec:greedy-random-approx-uniform}}
...
the paper.
\begin{prop}
\label{lem:H-count}Let $H$ be a fixed $r$-graph with an identified
...
$\left(\varepsilon,v\left(H\right)-1\right)$-quasirandom $r$-graph
on $n$ vertices, and let $F$ be a copy of the induced $r$-graph
$H\left[U\right]$ in $G$. Then, the number of completions of $F$
to a copy of $H$ is 
\[
\left(1\pm O\left(\varepsilon\right)\right)\left(1-\frac{i}{N}\right)^{e\left(H\right)-e\left(F\right)}\frac{n^{q-\left|U\right|}}{\left|\operatorname{Aut}\left(H,U\right)\right|}.
\]
In particular, the number of copies of $H$ in $G$ is 
\[
\left(1\pm O\left(\varepsilon\right)\right)\left(1-\frac{i}{N}\right)^{e\left(H\right)}\frac{n^{q}}{\left|\operatorname{Aut}\left(H\right)\right|}.
\]
\end{prop}
\begin{proof}
...
\[
\left(1\pm O\left(\varepsilon\right)\right)\left(1-\frac{i}{N}\right)^{e\left(H\right)-e\left(F\right)}n^{q-\left|V\right|}.
\]
...
\[
\left(1\pm O\left(\varepsilon\right)\right)\left(1-\frac{i}{N}\right)^{e\left(H\right)-e\left(F\right)-d_{H}\left(v\right)}n^{q-\left|V\right|-1}
\]
embeddings of $H-v$ extending $F$, where $d_{H}\left(v\right)$
is the number of edges of $H$ containing $v$. For each such embedding,
by $\left(\varepsilon,v\left(H\right)-1\right)$-quasirandomness,
...
\end{proof}
%
\begin{proof}[Proof of \ref{lem:greedy-random-approx-uniform}]
Each $G\left(S_{i}\right)$ has 
\[
\left(1\pm O\left(n^{-a}\right)\right)\left(1-\frac{i}{N}\right)^{Q}\frac{n^{q}}{q!}
\]
 copies of $K_{q}^{\left(r\right)}$, by $\left(n^{-a},q-1\right)$-quasirandomness
and \ref{lem:H-count}. We therefore have
\[
\Pr\left(\randS=S\right)=\prod_{i=0}^{\alpha N-1}\frac{1}{\left(1\pm O\left(n^{-a}\right)\right)\left(1-i/N\right)^{Q}n^{q}/q!},
\]
...
\frac{\Pr\left(\randS=S\right)}{\Pr\left(\randS=S'\right)} & \le\left(1+O\left(n^{-a}\right)\right)^{\alpha N}\\
 & \le\exp\left(O\left(n^{r-a}\right)\right)
\end{align*}
as desired.
\end{proof}
...
definitions we will show how to couple the clique removal process
with a simpler random hypergraph distribution.
\begin{defn}
For a partial system $S$, let $\Gnp Sp$ be the random distribution
on $q$-uniform hypergraphs where each hyperedge not conflicting with
$S$ (that is, not intersecting a hyperedge of $S$ in more than $r-1$
vertices) is included with probability $p$. So, if $\varnothing$
is the empty order-$n$ partial systems, then $\Gnp{\varnothing}p:=\Gnp np$
is the standard binomial random $r$-graph. Let $\GSnp Sp$ be the
distribution on partial systems obtained from $\Gnp Sp$ by considering
all hyperedges which intersect another hyperedge in more than $r-1$
vertices, and deleting all these hyperedges. Let $\rg Sm$ be the
partial system distribution obtained with $m$ steps of the clique
removal process starting from $G\left(S\right)$.
\end{defn}
Let $P=n^{r-q}\left(q-r\right)!$ so that ${n \choose q}P=\left(1+o\left(1\right)\right)N$.
For small $\alpha>0$, we can view $\GSnp S{\alpha P}$ as a ``bite''
...
and $S'\supseteq S$ implies $S'\in\mathcal{P}$. For fixed $h\ge q-1$
...
Let $\randS\in\rg S{\alpha N}$ and $\randS^{*}\in\GSnp S{\alpha P}$.
Then
\[
...
\end{lem}
\begin{proof}
Let $\randS^{*}\in\GSnp S{\alpha P}$ be obtained from $\randG\in\Gnp S{\alpha P}$.
...
of edges in $\randG$ is at most ${n \choose q}\alpha P\le\alpha N$,
...
and the clique removal process with this ordering produces a superset
of $\randS^{*}$.
...
with Bernstein-type tails which can be used to analyse $\GSnp S{\alpha P}$.
...
system, proving \ref{thm:PM-in-STS}. Recall that Steiner triple systems
have $q=3$ and $r=2$, so in the notation of \ref{sec:outline} (and
\ref{sec:nibble-TRP-coupling}) we have $Q=3$, $N={n \choose 2}/3=\left(1+o\left(1\right)\right)n^{2}/6$
and $P=1/n$.
...
\begin{itemize}
\item they could intersect in the unfixed hyperedge of the covering matching
...
have many perfect matchings. There are many interesting open questions
about random Steiner (triple) systems and perfect matchings that remain.
\begin{itemize}
...
have $\Omega\left(n^{2}\right)$ disjoint perfect matchings, but to
find $\left(1-o\left(1\right)\right)n/2$ disjoint perfect matchings
would require a new idea. For Latin squares, the property of being
decomposable into transversals is equivalent to the important property
of having an \emph{orthogonal mate}, which has a long history dating
back to Euler. More details can be found in \cite{Wan11}.
\item There is the obvious question of proving that $\left(q,r\right)$-Steiner
systems a.a.s. have perfect matchings, for all choices of $\left(q,r\right)$.
We believe this should be possible using the methods in this paper,
and the main difficulty probably lies in coming up with a suitable
absorber. If $r>2$ then there is the more general question of whether
...
and so on. A similar phenomenon occurs for random regular graphs (see
\cite[Section~9.5]{JLR00})
...
ideas, because the expected number of 7-vertex Steiner triple systems
(Fano planes) in the 3-uniform random hypergraph $\Gnp n{1/n}$ is
$\Theta\left(1\right)$.
...
\section{\label{sec:keevash}Counting completions of Steiner systems}
...
with minor adaptations of a proof by Keevash \cite{Kee15}. As in
\ref{sec:outline}, we consider $q$ and $r$ to be fixed, let $Q={q \choose r}$
and let $N={n \choose r}/Q$.

For a partial system $S\in\S_{\alpha N}^{n^{-a}}$, let $\Sext S$
be the number of Steiner systems that contain $S$. We want to determine
$\left|\ext S\right|=\left(N-\alpha N\right)!\left|\Sext S\right|$
up to a factor of $e^{n^{2-b}}$ (for some $b$).

First, we can get an upper bound via the entropy method. This proof
is almost exactly the same to the proof of \cite[Theorem~6.1]{Kee15},
and is very similar to the proof that appears in a paper by Linial
and Luria \cite{LL13} about Steiner triple systems. The reader may
refer to that paper for more detailed exposition and a brief introduction
to the notion of entropy.
\begin{thm}
\label{thm:number-of-completions-upper}For any $a>0$, any $\alpha\in\left[0,1\right]$,
and any $S^{*}\in\cSm{n^{-a},q-1}{\alpha N}$,
\[
\left|\Sext{S^{*}}\right|\le\left(\left(1+O\left(n^{-a}\right)\right)\left(\frac{1-\alpha}{e}\right)^{Q-1}{n-r \choose q-r}\right)^{N\left(1-\alpha\right)}.
\]
\end{thm}
\begin{proof}
...
Let $\coG=G\left(S^{*}\right)$. For each $e\in\coG$, let $\randS_{e}$
be the $q$-edge that includes $e$ in $\randS$. So, the sequence
$\left(\randS_{e}\right)_{e\in\coG}$ determines $\randS$. For any
ordering on the $r$-edges of $\coG$, we have
\[
H\left(\randS\right)=\sum_{e\in\coG}H\left(\randS_{e}\cond\left(\randS_{e'}\,\colon\,e'<e\right)\right).
\]
Now, a sequence $\lambda\in\left[0,1\right]^{E\left(\coG\right)}$
with all $\lambda_{e}$ distinct induces an ordering on the $r$-edges
of $\coG$, with $e'<e$ when $\lambda_{e'}>\lambda_{e}$. Let $\randR_{e}\left(\lambda\right)$
be an upper bound on the number of possible values for $\randS_{e}$
given the information $\left(\randS_{e'}\,\colon\,\lambda_{e'}>\lambda_{e}\right)$,
defined as follows. $\randR_{e}=1$ if $\lambda_{e'}>\lambda_{e}$
for any of the $r$-subsets $e'\subseteq\randS_{e}$ (because in this
case $\randS_{e}$ is determined), and otherwise $\randR_{e}$ is
1 plus the number of $q$-sets of vertices $X\ne\randS_{e}$ containing
$e$ such that 
\begin{itemize}
\item $f\in\coG$ for each $r$-subset $f\subseteq X$, and
\item $\lambda_{e'}<\lambda_{e}$ for each of the $r$-edges $e'\in\coG$
included in any of the $q$-edges of $\randS$ that include the $r$-subsets
in $\left\{ f:f\subseteq X,\left|f\right|=r\right\} \backslash\randS_{e}$.
\end{itemize}
Note that the number of edges $e'$ considered by the second bullet
point is $Q\left(Q-1\right)$ unless $\left|X\cap B\right|>r$ for
some $q$-edge $B$ in $\randS$ (in this case we say $X$ is ``bad'').
By \ref{lem:H-count}, the number of choices for $X$ is 
\[
\left(1\pm O\left(n^{-a}\right)\right)\left(1-\alpha\right)^{Q-1}\frac{n^{q-r}}{\left(q-r\right)!}=\left(1\pm O\left(n^{-a}\right)\right)\left(1-\alpha\right)^{Q-1}{n-r \choose q-r}
\]
 and the number of those that are bad is at most $\sum_{i=r+1}^{q}N{n \choose q-r-i}=O\left(n^{q-r-1}\right)$.

Now, note that for any $\lambda$ we have
\[
H\left(\randS_{e}\cond\left(\randS_{e'}\,\colon\,\lambda_{e'}>\lambda_{e'}\right)\right)\le\E\left[\log\randR_{e}\left(\lambda\right)\right].
\]
...
let 
\[
R_{e}^{S,\lambda}=\E\left[\randR_{e}\left(\randl\right)\cond\randS=S,\,\randl_{e}=\lambda,\,\randl_{e'}<\randl_{e}\textrm{ for all }e'\subseteq\randS_{e}\backslash\left\{ e\right\} \right].
\]
By the discussion above, and linearity of expectation, we have 
\begin{align*}
R_{e}^{S,\lambda} & =\left(\left(1\pm O\left(n^{-a}\right)\right)\left(1-\alpha\right)^{Q-1}{n-r \choose q-r}-O\left(n^{q-r-1}\right)\right)\lambda^{Q\left(Q-1\right)}\\
 & =\left(1\pm O\left(n^{-a}\right)\right)\left(1-\alpha\right)^{Q-1}{n-r \choose q-r}\lambda^{Q\left(Q-1\right)}.
\end{align*}
Now, by Jensen's inequality,
\[
\E\left[\log\randR_{e}\left(\randl\right)\cond\randS=S,\,\randl_{e}=\lambda,\,\randl_{e}=\lambda,\,\randl_{e'}<\randl_{e}\textrm{ for all }e'\subseteq\randS_{e}\backslash\left\{ e\right\} \right]\le\log R_{e}^{S,\lambda},
\]
and 
\[
\Pr\left(\randl_{e}=\lambda,\,\randl_{e'}<\randl_{e}\textrm{ for all }e'\subseteq\randS_{e}\backslash\left\{ e\right\} \cond\randl_{e}=\lambda\right)=\lambda^{Q-1},
\]
so
\[
\E\left[\log\randR_{e}\left(\randl\right)\cond\randS=S,\,\randl_{e}=\lambda\right]\le\lambda^{Q-1}\log R_{e}^{S,\lambda}+\left(1-\lambda^{Q-1}\right)\log1=\lambda^{2}\log R_{e}^{S,\lambda}.
\]
For any $S\in\Sext{S^{*}}$, we then have 
\begin{align*}
\E\left[\log\randR_{e}\left(\randl\right)\cond\randS=S\right] & \le\E\left[\lambda^{Q-1}\log R_{e}^{S,\randl_{e}}\right]\\
 & =\int_{0}^{1}\lambda^{Q-1}\log\left(\left(1\pm O\left(n^{-a}\right)\right)\left(1-\alpha\right)^{Q-1}{n-r \choose q-r}\lambda^{Q\left(Q-1\right)}\right)\d\lambda\\
 & =\frac{1}{Q}\left(\log\left(\left(1\pm O\left(n^{-a}\right)\right)\left(1-\alpha\right)^{Q-1}{n-r \choose q-r}\right)-\left(Q-1\right)\right),
\end{align*}
using the fact that $\int_{0}^{1}\lambda^{A-1}\log\left(C\lambda^{B}\right)\d t=A^{-1}\log C-A^{-2}B$
...
\log\left|\Sext{S^{*}}\right| & \le H\left(\randS\right)\\
 ...
 & \le\left(N-\alpha N\right)\frac{1}{Q}\left(\log\left(\left(1\pm O\left(n^{-a}\right)\right)\left(1-\alpha\right)^{Q-1}{n-r \choose q-r}\right)-\left(Q-1\right)\right),
\end{align*}
which is equivalent to the theorem statement.
\end{proof}
...
\[
\left|\ext{S^{*}}\right|\le\left|\Sext{S^{*}}\right|\le\left(\left(1+O\left(n^{-a}\right)\right)\left(\frac{1-\alpha}{e}\right)^{Q-1}{n-r \choose q-r}\right)^{N\left(1-\alpha\right)}\left(N-\alpha N\right)!.
\]
\end{thm}
To prove \ref{thm:number-of-completions-lower} we will need an analysis
of the clique removal process (see \ref{sec:Random-triangle-removal})
...
can be completed to an Steiner system.
\end{thm}
...
Let $h$ be as in \ref{thm:keevash}. Fix small $c$ (smaller than
$a\,b\left(a,h\right)$ in the notation of \ref{lem:triangle-removal-analysis}),
let $\varepsilon=n^{-c}$ and let $M=\left(1-\varepsilon\right)N$.
Let $\randS\in\RR\left(S,M-m\right)$ be the result of running the
clique removal process on $G\left(S\right)$ to build a partial system
extending $S$, until there are $M$ hyperedges. Let $\ord^{*}$ be
the set of $M$-hyperedge $\left(\varepsilon,h\right)$-quasirandom
ordered partial systems $S\in\cordm{\varepsilon,h}M$ extending $S$.
By \ref{lem:H-count}, for each $S\in\ord^{*}$, the number of copies
of $K_{q}^{\left(r\right)}$ in each $G\left(S_{i}\right)$ is $\left(1\pm O\left(n^{-c}\right)\right)\left(1-i/N\right)^{Q}n^{q}/q!$
by quasirandomness. So,
\[
\Pr\left(\randS=S\right)\le\prod_{i=\alpha N}^{M}\frac{1}{\left(1-O\left(n^{-c}\right)\right)\left(1-i/N\right)^{Q}n^{q}/q!}.
\]
...
so 
\begin{align*}
\left|\ord^{*}\right| & \ge\left(1-o\left(1\right)\right)\prod_{i=\alpha N}^{M}\left(1-O\left(n^{-c}\right)\right)\left(1-\frac{i}{N}\right)^{Q}n^{q}/q!.\\
 & =\left(\left(1-O\left(n^{-c}\right)\right)\frac{n^{q}}{q!}\right)^{\left(1-\alpha\right)N}\exp\left(Q\sum_{i=\alpha N}^{M}\log\left(1-\frac{i}{N}\right)\right).
\end{align*}
Now, note that 
\[
...
\begin{align*}
Q\sum_{i=\alpha N}^{M}\log\left(1-\frac{i}{N}\right) & =QN\int_{\alpha}^{\left(1-\varepsilon\right)}\log\left(1-t\right)\d t+O\left(\log n\right)\\
 & =QN\int_{\varepsilon}^{\left(1-\alpha\right)}\log p\d p+O\left(\log n\right)\\
 & =QN\left(\left(1-\alpha\right)\left(\log\left(1-\alpha\right)-1\right)-\varepsilon\left(\log\varepsilon-1\right)\right)+O\left(\log n\right),\\
\exp\left(Q\sum_{i=\alpha N}^{M}\log\left(1-\frac{i}{N}\right)\right) & =\left(\left(1+O\left(n^{-c}\log n\right)\right)\frac{1-\alpha}{e}\right)^{QN\left(1-\alpha\right)}.
\end{align*}
For $b<c$, it follows that
\begin{align*}
\left|\ord^{*}\right| & \ge\left(\left(1-O\left(n^{-b}\right)\right)\frac{n^{q}\left(1-\alpha\right)^{Q}}{q!e^{Q}}\right)^{\left(1-\alpha\right)N}\\
 & =\left(\left(1-O\left(n^{-b}\right)\right)\left(\frac{1-\alpha}{e}\right)^{Q-1}{n-r \choose q-r}\right)^{\left(1-\alpha\right)N}\left(N-\alpha N\right)!
\end{align*}
By \ref{thm:keevash} (assuming $b$ is small enough) it follows that
\[
\left|\ext{S^{*}}\right|\ge\left(\frac{n\left(1-\alpha\right)^{2}}{e^{2}}\left(1-O\left(n^{-b}\right)\right)\right)^{\left(1-\alpha\right)N}\left(N-\alpha N\right)!.\tag*{\qedhere}
\]
\end{proof}
...
\section{\label{sec:Random-triangle-removal}Random clique removal}
...
In this section we give a very simple analysis of the clique removal
...
As in \ref{sec:outline}, we consider $q$ and $r$ to be fixed, let
$Q={q \choose r}$ and let $N={n \choose r}/Q$.

As introduced in \ref{sec:outline}, the clique removal process is
defined as follows. We start with a graph $G$ with say $N-Qm$ edges,
then iteratively delete (the edges of) a copy of $K_{q}^{\left(r\right)}$
chosen uniformly at random from all copies of $K_{q}^{\left(r\right)}$
in the remaining graph. Let 
\[
...
the sequence with no copies of $K_{q}^{\left(r\right)}$, then let
$\randG\left(i\right)=\randG\left(\randM\right)$ for $i\ge\randM$.

Our objective in this section is to show that if $G$ is quasirandom
then the clique removal process is likely to maintain quasirandomness
...
\end{thm}
Note that $K_{n}^{\left(r\right)}$ is $\left(O\left(1/n\right),h\right)$-quasirandom
for any $h$, so in particular when we start the clique removal process
from $G=K_{n}^{\left(r\right)}$ it typically runs almost to completion.
...
For a set $A$ of $\left(r-1\right)$-sets of vertices with $\left|A\right|\le h$,
let $\randY_{A}\left(i\right)=\left|\bigcap_{W\in A}N_{\randG\left(i\right)}\left(W\right)\right|$.
...
copies of $K_{q}^{\left(r\right)}$ is
\[
\randQ\left(i\right)=\left(1\pm O\left(e\left(i\right)\right)\right)p^{Q}\left(i\right)n^{q}/q!>0.
\]
...
remove a copy of $K_{q}^{\left(r\right)}$ containing an $r$-edge
$\left\{ x\right\} \cup W$ for some $W\in A$. Now, for each $W\in A$,
the number of copies of $K_{q}^{\left(r\right)}$ in $\randG\left(i\right)$
containing the edge $\left\{ x\right\} \cup W$ is $\left(1\pm O\left(e\left(i\right)\right)\right)p^{Q-1}\left(i\right)n^{q-r}/\left(q-r\right)!$
by \ref{lem:H-count}. The number of copies of $K_{q}^{\left(r\right)}$
containing $\left\{ x\right\} \cup W$ and $\left\{ x\right\} \cup W'$
for two different $W,W'\in A$ is at most $O\left(n^{q-r-1}\right)$.
So,
\begin{align*}
\Pr\left(x\notin N_{A}\left(i+1\right)\right) & =\frac{1}{\randQ\left(i\right)}\left(\sum_{W\in A}\left(1\pm O\left(e\left(i\right)\right)\right)p^{Q-1}\left(i\right)n^{q-r}/\left(q-r\right)!-O\left(n^{q-r-1}\right)\right)\\
...
\end{align*}
Note also that we have the bound $\Delta\randY_{A}\left(i\right)\le q-1=O\left(1\right)$
