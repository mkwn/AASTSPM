...
(via $\Gnp n{\alpha/n}$ and \ref{lem:bite-transfer}) with extremely
high probability.
\begin{defn}
...
9 vertices the \emph{external vertices}. We say the hyperedges containing
only external vertices are \emph{external hyperedges}. Note that an
absorber already has a perfect matching on its 12 vertices (the first
4 edges; we call this the \emph{covering }matching), but it also has
a perfect matching on its 9 vertices not including $x,y,z$ (the last
3 edges; we call this the \emph{non-covering }matching). See \ref{fig:absorber}.
\end{defn}
\begin{figure}[h]
\begin{center}
...
Absorbers are the basic building block for a larger structure which
will eventually allow us to turn an almost-perfect matching into a
perfect matching. The relative positions of the absorbers in this
...
vertex set by putting a hyperedge for each length-2 path in $R'$.
Note that an edge in $R$ can be uniquely extended to a length-2 path
in $R'$, so $T$ has at most $120n$ hyperedges. Moreover, a matching
in $R$ can always be extended to a factor of length-2 paths in $R'$,
...
relative positions of the absorbers, its edges are not actually in
the absorbing structure).
\end{defn}
Note that an absorbing structure with a flexible set of size $n$
has $10n+9\times120n=O\left(n\right)$ vertices and at most $7\times120n=O\left(n\right)$
edges. An absorbing structure has the same crucial property as the
resilient template that defines it: if we remove half of the vertices
of the flexible set then the remaining hypergraph has a perfect matching.
Indeed, after this removal we can find a perfect matching $M$ in
the template, then we can use the covering matching of the absorber
on each hyperedge of $M$ and the non-covering matching for the absorber
on each other hyperedge. The existence of an absorbing structure,
in addition to some very weak pseudorandomness conditions, allows
us to find perfect matchings in a Steiner triple system, as follows.
\begin{lem}
...
\ref{cond:density} to repeatedly choose an edge induced by the remaining
unmatched vertices in $V\backslash V\left(H\right)$ until there are
only $3\floor{\delta^{3}n}$ such vertices remaining unmatched. (This
means we are choosing $m=\left(n'-3\floor{\delta^{3}n}\right)/3=n/3-O\left(\delta n\right)$
...
vertices in $Z$ used so far, to extend our matching. We can then
repeatedly apply \ref{cond:density} to the uncovered vertices in
$Z$ to extend our matching to cover half of $Z$. By the crucial
property of an absorbing structure, we can find a perfect matching
on the remaining vertices, completing our perfect matching of $S$.
...
Let $\randS^{*}\in\GSnp n{\alpha/N}$ be obtained from $\randG\in\Gnp n{\alpha/N}$,
and consider a set $W$ of $\floor{\delta n}$ vertices outside $Z$.
Let $\randY$ be the number of hyperedges of $\randS^{*}$ with one
vertex in $W$ and two vertices in $Z$. That is to say, $\randY$
is the number of such hyperedges in $\randG$ that are \emph{isolated}
in the sense that they do not intersect any other hyperedge of $\randG$
in more than one vertex. There are $\Theta\left(\left(\delta n\right)^{2}n\right)=\Theta\left(\delta^{2}n^{3}\right)$
...
at just the first $\alpha N$ hyperedges, but the key observation
is that in a random ordered Steiner triple system, by symmetry the
first $\alpha N$ hyperedges have the same distribution as the hyperedges
corresponding to any other choice of $\alpha N$ indices.
...
the third condition in \ref{lem:PM-via-absorbers} is then satisfied.
...
to do this by na\"ively applying \ref{lem:triangle-removal-transfer}
as in \ref{subsec:degree,subsec:density}, because the probability
that a vertex is isolated in $\rg n{\alpha N}$ is already too large.
Instead we must use \ref{lem:triangle-removal-transfer} in its full
generality, conditioning on the quasirandomness of the first few steps
of the triangle removal process.
...
when combined with three hyperedges of $S$ incident to $x,y,z$.
Let $\randY$ be the maximum size of a hyperedge-disjoint collection
of absorber-extensions in $\randS^{*}$. That is, it is the maximal
size of a collection of disjoint isolated absorber-extensions in $\randG$.
This particular choice of random variable is crucial, and allows us
to use \ref{thm:bernstein-type}. (The idea comes from a similar random
variable used by Bollob\'as \cite{Bol88}). Adding a hyperedge to
$\randG$ can increase the size of a maximal collection of disjoint
absorber-extensions by at most one, and removing a hyperedge can cause
at most three absorber-extensions in a maximal disjoint collection
to become isolated. So, changing the presence of a hyperedge in $\randG$
can change $\randY$ by at most 3, as in \ref{subsec:degree,subsec:density}.
\begin{claim}
If $h$ is large enough and $\alpha$ is small enough, then $\E\randY=\Theta\left(n^{2}\right)$.
\end{claim}
\begin{proof}
...
ways to do this). Then, the number of ways to choose the remaining
four hyperedges of a candidate absorber-extension is precisely the
number of copies in $G\left(S\right)$ of a certain graph $F$ rooted
on the vertices of $e_{x},e_{y},e_{z}$. (Specifically, $F$ is the
graph obtained by taking the external hyperedges of the hypergraph
in \ref{def:absorber} and replacing each hyperedge with a triangle
on its vertex set). Provided $h$ is large enough, by \ref{lem:H-count}
the number of suitable copies of $F$ is $\left(1-\alpha\right)^{O\left(1\right)}n^{3}=\Theta\left(n^{3}\right)$.
...
\]
So, $\E\randX=\Theta\left(\alpha^{7}n^{2}\right)$. Now, there are
several possibilities for a hyperedge-intersecting pair of distinct
absorber-extensions.
\begin{itemize}
\item They could intersect in the external hyperedge of the covering matching
...
\item They could intersect in one hyperedge of the non-covering matching.
There are $\Theta\left(\left(\alpha n\right)^{4}n^{5}\right)$ possible
...
\item They could intersect in two hyperedges of the non-covering matching.
There are $\Theta\left(\left(\alpha n\right)^{3}n^{4}\right)$ possible
such pairs, and each occurs with probability $\Theta\left(\left(\alpha/n\right)^{6}\right)$,
...
externally vertex-disjoint absorber-extensions rooted on $x,y,z$.
We can find these greedily; each vertex is involved in only $O\left(n\right)$
hyperedges of $\randS$, so deleting $O\left(1\right)$ vertices can
delete at most $O\left(n\right)$ hyperedges. By the union bound,
\ref{lem:bite-transfer} and \ref{lem:triangle-removal-transfer}
(with $\mathcal{Q}$ as defined above), it follows that in a random
Steiner triple system, a.a.s. every triple of vertices has $\Omega\left(n\right)$
externally vertex-disjoint absorbers.

If a Steiner triple system has this property, then we can greedily
build our absorbing structure, as follows. Recalling that $\left|Z\right|=3\floor{\delta^{3}n}=O\left(\delta^{3}n\right)$,
choose a resilient template $T$ with $O\left(\delta^{3}n\right)$
hyperedges, on $O\left(\delta^{3}n\right)$ vertices of $\randS$,
such that the flexible set is $Z$. Noting that an absorber has $O\left(1\right)$
non-rooted vertices, we can greedily find an absorber in $\randS$
on each hyperedge of $H$, in an externally vertex-disjoint fashion.
The entire absorbing structure then has $O\left(\delta^{3}n\right)\le\delta n$
...
vertices are in distinct hyperedges of $\randM$ (if they were in
the same hyperedge that hyperedge would intersect $e$ in two vertices,
violating the defining property of a Steiner triple system). By Jensen's
inequality,
\[
...
near optimal discrepancy. See \cite{LL15} for some theorems and conjectures
about discrepancy of Latin squares.
\end{itemize}
